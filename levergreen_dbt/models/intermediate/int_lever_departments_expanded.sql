...
jobs_outline_unnested as (
    select
        ...
        trim(
            department_ids_unnested.department_name
        ) as department_name,
    ...
    from
        ...
        unnest(
            string_to_array(
                lever_jobs_outline.department_names,
                u & '\2013'
            )
        ) with ordinality as department_ids_unnested(
            department_name,
            department_level
...
departments_aggregated as (
    select
        ...
        max(
            case
                when department_level = 1 then department_name
            end
        ) as primary_department,
        max(
            case
                when department_level = 2 then department_name
            end
        ) as secondary_department,
        max(
            case
                when department_level = 3 then department_name
            end
        ) as tertiary_department,
        max(
            case
                when department_level = 4 then department_name
            end
        ) as quaternary_department
    from
...
select
    distinct jobs_outline_unnested.id,
...
from
    ...
    on jobs_outline_unnested.id = departments_aggregated.id
