with greenhouse_jobs_outline as (
    select
        *
    from
        {{ ref('stg_greenhouse__jobs_outline') }}
),
greenhouse_job_departments as (
    select
        *
    from
        {{ ref('stg_greenhouse__job_departments') }}
),
jobs_outline_unnested as (
    select
        ...
        lower(
            greenhouse_jobs_outline.location
        ) like '%remote%' as is_remote,
        department_ids_unnested.department_id
    from
        greenhouse_jobs_outline,
        unnest(string_to_array(department_ids, ',')) as department_ids_unnested(department_id)
),
outline_joined_to_depts as (
    select
        ...
        case
            when greenhouse_job_departments.department_category = 'level-1' then greenhouse_job_departments.department_name
        end as secondary_department,
        case
            when greenhouse_job_departments.department_category = 'level-2' then greenhouse_job_departments.department_name
        end as tertiary_department,
        case
            when greenhouse_job_departments.department_category = 'level-3' then greenhouse_job_departments.department_name
        end as quaternary_department
    from
        jobs_outline_unnested
        left join greenhouse_job_departments
        on jobs_outline_unnested.source = greenhouse_job_departments.source
...
departments_aggregated as (
    ...
    from
        outline_joined_to_depts
    group by
        1
)
select
    distinct outline_joined_to_depts.id,
...
from
    outline_joined_to_depts
    inner join departments_aggregated
    on outline_joined_to_depts.id = departments_aggregated.id
