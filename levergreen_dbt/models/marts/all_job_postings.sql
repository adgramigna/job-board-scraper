with all_postings as (
    select
        *
    from
        {{ ref('int_greenhouse_departments_expanded_with_outline') }}
    union all
    select
        *
    from
        {{ ref('int_lever_departments_expanded') }}
),
...
all_postings_with_earliest_and_latest as (
    ...
    from
        all_postings
        inner join get_earliest_and_latest_dates as earliest_and_latest
        on all_postings.full_opening_link = earliest_and_latest.full_opening_link)
    select
        *,
        case
        ...
        case
            ...
            or lower(opening_title) like '%accounting%' then 'Finance & Accounting'
            when lower(primary_department) like '%operations%'
            or lower(secondary_department) like '%operations%'
            or lower(opening_title) like '%operations%' then 'Operations'
            when lower(primary_department) like '%product%'
            or lower(secondary_department) like '%product%'
            or lower(opening_title) like '%product%' then 'Product'
            else 'Other'
        end as opening_category
    from
        all_postings_with_earliest_and_latest
